...
namespace web_platform.Service
{
    public class SecurityIssuePostService : ISecurityIssuePost
    {
        ...
        public async Task<SecurityIssuePostReply> Update(int securityIssuePostReplyId, string content)
        {
            var replyToFind = await _umbracoDbContext.SecurityIssuePostReplies.Where((s) => s.Id == securityIssuePostReplyId).FirstOrDefaultAsync();
            if (replyToFind != null)
            {
                replyToFind.Content = content;
                await _umbracoDbContext.SaveChangesAsync();
            }
